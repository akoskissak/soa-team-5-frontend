...
export class BlogListComponent implements OnInit {
  ...
  goToCreateBlog(): void {
   ...
   toggleLike(postId: string | undefined): void { 
    ...
    this.blogService.toggleLike(postId, this.currentUserId).subscribe({
      next: () => {
        ...
        console.log(`Lajk/Unlajk uspešno za post ${postId}. Novi broj lajkova: ${post.LikesCount}`);
      },
      error: (err) => {
   ...
   addComment(post: Post): void { // <-- Sad prima CEO post objekat
    ...
    this.blogService.addComment(post.ID, commentText, this.currentUserId, this.currentUsername).subscribe({
